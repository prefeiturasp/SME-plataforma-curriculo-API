...
module Api
  module V1
    class OmniauthCallbacksController < DeviseTokenAuth::ApplicationController
      ...
      def omniauth_params
        unless defined?(@_omniauth_params)
          if request.env['omniauth.params'] && request.env['omniauth.params'].any?
            ...
            @_omniauth_params = params.slice('omniauth_window_type', 'auth_origin_url', 'resource_class', 'namespace_name','origin', 'SAMLResponse')
          else
            @_omniauth_params = {}
          end
        end
      ...
      def devise_mapping
        # CHANGED: if omniauth_params
        if omniauth_params.present?
